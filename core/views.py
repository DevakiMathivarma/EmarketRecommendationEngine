...
from django.shortcuts import render, get_object_or_404
from .models import Product, ProductRecommendation, UserProductInteraction, ParentCategoryAssignment
...
def product_detail(request, pk):
    ...
    recommended = []
    also_bought = []
    MAX_SHOW = 6   # number of items to show; adjust as needed

    try:
        pr = getattr(product, "recommendation", None)

        # --- recommended (existing behavior) ---
        if pr and pr.recommended_ids:
            rec_ids = [int(x) for x in pr.recommended_ids]
            preserved = {id_: idx for idx, id_ in enumerate(rec_ids)}
            qs = Product.objects.filter(id__in=rec_ids)
            recommended = sorted(qs, key=lambda x: preserved.get(x.id, 999))

        # --- also_bought with category-prioritization (your existing logic) ---
        if pr and getattr(pr, "also_bought_ids", None):
            ab_ids = [int(x) for x in pr.also_bought_ids]
            if ab_ids:
                preserved_ab = {id_: idx for idx, id_ in enumerate(ab_ids)}

                # 1) same subcategory first
                cat = getattr(product, "category", None)
                chosen = []
                if cat:
                    same_cat_qs = Product.objects.filter(id__in=ab_ids, category=cat)
                    same_cat = sorted(same_cat_qs, key=lambda x: preserved_ab.get(x.id, 999))
                    for p in same_cat:
                ...
                if len(chosen) < MAX_SHOW:
                    try:
                        pa = ParentCategoryAssignment.objects.filter(category=product.category).select_related("parent").first()
                        parent_obj = pa.parent if pa and pa.parent else None
                    except Exception:
                        parent_obj = None

                    if parent_obj:
                        same_parent_qs = Product.objects.filter(
                            id__in=ab_ids,
                            category__parent_assignment__parent=parent_obj
                        )
                        same_parent = sorted(same_parent_qs, key=lambda x: preserved_ab.get(x.id, 999))
                        for p in same_parent:
                            if len(chosen) >= MAX_SHOW:
                                break
                            if p.id != product.id and p not in chosen:
                                chosen.append(p)

                # 3) fill remaining from overall also_bought order
                if len(chosen) < MAX_SHOW:
                    remaining_ids = [i for i in ab_ids if i not in {q.id for q in chosen} and i != product.id]
                    if remaining_ids:
                        rem_qs = Product.objects.filter(id__in=remaining_ids)
                        rem_list = sorted(rem_qs, key=lambda x: preserved_ab.get(x.id, 999))
                        for p in rem_list:
                            if len(chosen) >= MAX_SHOW:
                                break
                            chosen.append(p)
        ...
        # --- FALLBACK: if also_bought is empty, show "Popular in this category" ---
        if not also_bought:
            # try same subcategory popular by interaction count -> rating -> newest
            cat = getattr(product, "category", None)
            if cat:
                # annotate with count of interactions (order+cart+view all counted; adjust filter if you want order only)
                popular_qs = Product.objects.filter(category=cat).exclude(id=product.id).annotate(
                    interactions=Count('userproductinteraction')
                ).order_by('-interactions', '-rating', '-id')[:MAX_SHOW]

                also_bought = list(popular_qs)

            # if category missing or still empty, try parent category popular
            if not also_bought:
                try:
                    pa = ParentCategoryAssignment.objects.filter(category=product.category).select_related("parent").first()
                    parent_obj = pa.parent if pa and pa.parent else None
                ...
                if parent_obj:
                    popular_parent_qs = Product.objects.filter(
                        category__parent_assignment__parent=parent_obj
                    ).exclude(id=product.id).annotate(
                        interactions=Count('userproductinteraction')
                    ).order_by('-interactions', '-rating', '-id')[:MAX_SHOW]
                    also_bought = list(popular_parent_qs)

            # final fallback: site-wide popular
            if not also_bought:
                site_popular_qs = Product.objects.exclude(id=product.id).annotate(
                    interactions=Count('userproductinteraction')
                ).order_by('-interactions', '-rating', '-id')[:MAX_SHOW]
                also_bought = list(site_popular_qs)

    except Exception:
        # Fail-safe: keep lists as empty or previously filled
        recommended = recommended or []
        also_bought = also_bought or []

    # cart snapshot & quantity (unchanged)
    cart = request.session.get('cart', {})
    total = sum(item['price'] * item['qty'] for item in cart.values())
